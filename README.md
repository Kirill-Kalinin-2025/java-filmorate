...
1. **Дружба требует подтверждения** -  
**PENDING** - запрос на дружбу отправлен, но еще не подтвержден, **CONFIRMED** - дружба подтверждена обоими пользователями.
...
```sql
user_id BIGINT FK >- users.id    # Внешний ключ на пользователя (отправитель)
friend_id BIGINT FK >- users.id  # Внешний ключ на друга (получатель)
status VARCHAR(10)               # Статус: PENDING или CONFIRMED
created_at TIMESTAMP             # Дата отправки запроса
confirmed_at TIMESTAMP           # Дата подтверждения (NULL если PENDING)
